...
import { faArrowLeft, faTrash, faCheck, faDollarSign } from '@fortawesome/free-solid-svg-icons';
...
const Cart = () => {
  ...
  const handleRemoveCartItem = (artworkId) => {
    const updatedCartItems = cartItems.filter((item) => item.artworkId !== artworkId);
  ...
  const renderCartItems = () => {
      ...
      return (
        ...
        >
          <div className='card-body'>
            ...
            <p className='card-text'>Start adding items to your cart to make a purchase.</p>
          ...
          <div className='card-body'>
            ...
            <div className='d-flex justify-content-center'>
              {limitedCartItems.map((item) => (
                <div className='card mx-2' key={item.artworkId} style={{ width: '18rem' }}>
                  <img src={item.image} className={`card-img-top ${styles.cardImage}`} alt={item.title} />
                  <div className='card-body'>
                    <h5 className='card-title'>{item.title}</h5>
                    <p className='card-text'>Price: {item.price} USD</p>
                    <button className={styles.removeButton} onClick={() => handleRemoveCartItem(item.artworkId)}>
    ...
    return (
      <div className='d-flex justify-content-center' style={{ marginTop: '34px' }}>
        {cartItems.map((item) => (
          <div className='card mx-2' key={item.artworkId} style={{ width: '15rem' }}>
            <img src={item.image} className={`card-img-top ${styles.cardImage}`} alt={item.title} />
            <div className='card-body'>
              <h5 className='card-title'>{item.title}</h5>
